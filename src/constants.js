export const EXPENSE_CATEGORIES = [
  'Food','Groceries','Transport','Fuel','Rent','Utilities','Shopping','Entertainment','Health','Insurance','Education','Travel','Subscriptions','Gifts','Investments','Other'
];
export const INCOME_CATEGORIES = [
  'Salary','Bonus','Freelance','Interest','Dividends','Rental Income','Refund','Gift','Other'
];
export const PAYMENT_METHODS = [
  'Cash','UPI','Credit Card','Credit Card UPI','Sodexo / Meal Card','Debit Card','Bank Transfer'
];
export const STORAGE_KEYS = {
  tx: 'fintrack.transactions.v1',
  budgets: 'fintrack.budgets.v1',
  settings: 'fintrack.settings.v1'
};
export const BACKUP_FREQUENCIES = [
  'Never','Daily','Weekly','Every 2 weeks','Monthly'
];
